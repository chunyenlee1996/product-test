    ...
    <VRow>
      ...
      <VCol cols="12">
        <VForm v-model="valid" @submit.prevent="register">
          ...
          <VTextField ref="elPassword" v-model="form.password" label="密碼" counter maxlength="20" type="password"
            :rules="[rules.required, rules.min, rules.max, rules.confirm]"
            @update:modelValue="elPasswordConfirm.validate()"></VTextField>
          <VTextField ref="elPasswordConfirm" v-model="form.confirmPassword" label="確認密碼" counter maxlength="20"
            type="password" :rules="[rules.required, rules.min, rules.max, rules.confirm]"
