  ...
  VList(nav)
    template(v-for="navItem in navItems" :key="navItem.to")
      VListItem(v-if="navItem.show" :to="navItem.to")
        ...
        template(#append)
          VBadge(v-if="navItem.to === '/cart'" color="success" :content="cart.toString()")
...
VAppBar(color="primary")
  VContainer.d-flex.align-center
    ...
    template(v-if="!isMobile")
      template(v-for="navItem in navItems" :key="navItem.to")
        VBtn(v-if="navItem.show" variant="text" :prepend-icon="navItem.icon" :to="navItem.to") {{ navItem.text }}
          VBadge(v-if="navItem.to === '/cart'" color="success" :content="cart.toString()" floating)
